...
public class Mallet {
    ...
    private void enforceBoundaries() {
        ...
        // Top/Bottom boundaries with goal protection
        if (isPlayer1) {
            // Player 1 (bottom) - can't enter goal area
            if (y - radius < Constants.TABLE_Y) {
                y = Constants.TABLE_Y + radius;
                velocityY = 0;
            }
            if (y > Constants.TABLE_Y + Constants.TABLE_HEIGHT - 100) {
                y = Constants.TABLE_Y + Constants.TABLE_HEIGHT - 100;
                velocityY = 0;
            }
        } else {
            // Player 2/AI (top) - can't enter goal area
            if (y + radius > Constants.TABLE_Y + Constants.TABLE_HEIGHT) {
                y = Constants.TABLE_Y + Constants.TABLE_HEIGHT - radius;
                velocityY = 0;
            }
            if (y < Constants.TABLE_Y + 100) {
                y = Constants.TABLE_Y + 100;
                velocityY = 0;
            }
        }
    }
